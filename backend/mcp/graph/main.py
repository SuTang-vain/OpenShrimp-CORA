...
app = FastAPI(title="MCP-Graph Service", version="0.1.0")
...
TOOLS: List[ToolInfo] = [
    ToolInfo(
        name="graph_explain",
        description="Explain relationships in knowledge graph",
        schema={
            "type": "object",
            "properties": {
                "query": {"type": "string"},
                "mode": {"type": "string", "enum": ["neighbors", "shortest"], "default": "neighbors"},
...
async def invoke_tool(req: InvokeRequest) -> InvokeResponse:
    ...
    async with httpx.AsyncClient(timeout=timeout_s) as client:
        try:
            r = await client.post(f"{cora_url}/api/graph/query", json=payload)
            r.raise_for_status()
            data = r.json()
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("backend.mcp.graph.main:app", host="0.0.0.0", port=int(os.getenv("PORT", "8002")))